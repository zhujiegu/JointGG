z_statistic <- function(model_fit, GH_level_z){
  ...
  treat_list <- lapply(1:GH_level_z^b_dim, function(row){
    ...
    ##########################
    # mean_t
    mean_tt <- mean_t(mu, sigma, q)
    ######################
    # gradient w.r.t beta_mu
    mu_grt <- mean_tt
    # grad(function(mu) mean_t(mu, sigma, q), mu)
    ...
    # grad(function(sig) mean_t(mu, sig, q), sigma)
    # gradient w.r.t beta_q
    q_grt <- grad(function(q) mean_t(mu, sigma, q), q)
...
mean_t <- function(mu, sigma, q){
  t <- exp(mu-sigma*log(q^2)/q)* gamma(q^(-2)*(1+q*sigma))/gamma(q^(-2))
  return(t)
}